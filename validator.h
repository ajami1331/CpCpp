...
void Red(FILE* file) { fprintf(file, "\033[1;31m"); }

void Green(FILE* file) { fprintf(file, "\033[1;32m"); }

void Reset(FILE* file) { fprintf(file, "\033[0m"); }
...
void Process(bool validateTestCases) {
  ...
  for (int testCase = 1; testCase <= testCaseCnt; testCase++) {
    ...
    string outputFileForTestcaseName =
        testCaseDir + to_string(testCase) + ".out";
    string validFileForTestcaseName =
    ...
    bool success =
        CompareFiles(validFileForTestcaseName, outputFileForTestcaseName);
    if (success) {
      passedCnt++;
      Green(stderr);
