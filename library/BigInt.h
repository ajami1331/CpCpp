...
struct BigInt {
  ...
  void operator=(long long v) {
    ...
    if (v < 0) sign = -1, v = -v;
    z.clear();
    for (; v > 0; v = v / bigint_base) z.push_back(v % bigint_base);
  }
  ...
  friend BigInt Sqrt(const BigInt& a1) {
    ...
    for (int j = n / 2 - 1; j >= 0; j--) {
      for (;; --q) {
        BigInt r1 =
            (r - (res * 2 * bigint_base + q) * q) * bigint_base * bigint_base +
            (j > 0 ? (long long)a.z[2 * j - 1] * bigint_base + a.z[2 * j - 2]
                   : 0);
        if (r1 >= 0) {
          r = r1;
          break;
      ...
      if (j > 0) {
        ...
        q = ((long long)d1 * bigint_base * bigint_base +
             (long long)d2 * bigint_base + d3) /
            (firstDigit * 2);
      }
    }
  ...
  long long LongValue() const {
    long long res = 0;
    for (int i = z.size() - 1; i >= 0; i--) res = res * bigint_base + z[i];
    return res * sign;
  }
  ...
  static vector<int> Convertbigint_base(const vector<int>& a, int old_digits,
                                        int new_digits) {
